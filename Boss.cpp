...
Boss::Boss()
{
        ...
        isRunning = isAttacking = isAttacked = isTakinghit = isDead = causeDamage = cantAttacked = false;
        isIdling = true;
        BossBox.x = x_pos;
        BossBox.y = y_pos;
}
...
{
        if(HP <= 0) return;
        SDL_Rect RenderQuad = {BossBox.x - camX - 50, BossBox.y - camY - 40, HP * 10, HpBox.h / 2};
